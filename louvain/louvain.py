...
class Louvain:
    ...
    def iterate(self):
        ...
        if modified:
            ...
            G= self.coarse_grain_graph
            cmp= self.generate_community_map(
            self.community_history)
            partition = self.invert_community_map(cmp)        
            k=0.3
            node_size=60
            alpha=1
            community_map = {}
            ...
            indexed = [list(G.nodes).index(node) for node in G]
            #indexed = [random.random() for i in range(len(partition))]
            plt.axis("off")
           
            nx.draw_networkx(G, pos=pos,with_labels=False, node_color=indexed, node_size=node_size, alpha=0.2,cmap=cmap)
